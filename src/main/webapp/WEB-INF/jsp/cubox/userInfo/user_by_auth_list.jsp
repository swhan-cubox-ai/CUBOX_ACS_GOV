...
$(function() {
	...
	modalPopup ("authListPopup", "권한그룹 검색", 600, 705);
	...
	/* $("#authList").on("click", function(event){
		$("#authListPopup").PopupWindow("open");
	}); */	
	...
	$("input[name=chkAuthAll]").click(function() {
		if( $(this).prop("checked") ){
	...
	$("#srchAuthNm").keyup(function(e){if(e.keyCode == 13) fnAuthPopup('B');});
});
...
function userSearch() {
	...
	if(fnIsEmpty($("#srchAuthGroupNm").val()) && fnIsEmpty($("#srchNm").val()) && fnIsEmpty($("#srchCdNo").val())
	...
	$("#srchPage").val("1");
	frmSearch.action = "/userInfo/userListByAuth.do";
	frmSearch.submit();
}
...
function excelDownLoad() {
	...
	frmSearch.action = "/userInfo/userListByAuthExcel.do"
	frmSearch.submit();
				...
				if( status == "success") {
					if( returnData.list != null && returnData.list.length > 0 ) {
						for(var i=0; i < returnData.list.length; i++ ) {
							strHtml += "<tr><td>"
									+ "<input type='checkbox' name='chkAuth' value='"+ returnData.list[i].authorGroupId +"' class='mb_5'>"
									+ "</td><td>"
									+ returnData.list[i].authorGroupNm
									+ "</td></tr>";
						}
					}
					$("#tbAuthList").html(strHtml);
					if(gb == "A") {
						$("#authListPopup").PopupWindow("open");
					}
...
function selAuth() {
	var authCd = "";
	var authNm = "";
	$('input:checkbox[name="chkAuth"]').each(function() {
		if(this.checked) {
			authCd += $(this).val() + ",";
			authNm += $(this).parent().parent().eq(0).text() + ",";
		}
	});
	
	$("#srchAuthGroupCd").val(authCd);
	$("#srchAuthGroupNm").val(authNm);
	cancel2();
}

function cancel2() {
	$("input[name=chkAuthAll]").prop("checked", false);
	$("input[name=chkAuth]").prop("checked", false);
	$("#authListPopup").PopupWindow("close");
}
</script>
...
<div class="search_box mb_20">
	<div class="search_in_bline">
		<div class="comm_search mr_30">
			<input type="text" id="srchAuthGroupNm" name="srchAuthGroupNm" class="w_200px input_com" readonly placeholder="권한그룹명 " value='<c:out value="${param.srchAuthGroupNm}"/>'>
			<input type="hidden" id="srchAuthGroupCd" name="srchAuthGroupCd" value='<c:out value="${param.srchAuthGroupCd}"/>'>
		</div>
		<div class="comm_search">
			<div class="search_btn2" onclick="fnAuthPopup('A')"></div>
		</div>
...
<div class="tb_outbox">
	<table class="tb_list">
		...
		<c:forEach items="${userInfoList}" var="ulist" varStatus="status">
			<tr>
				<td><c:out value="${ulist.author_group_nm}" /></td>
...
<div id="excelDownloadPopup" class="example_content">
	<div class="popup_box">
		...
		<div class="search_box_popup mb_20">
			<div class="search_in">
				<div class="mr_m3">
					<input type="checkbox" name="excelColumn" id="author_group_nm" value="author_group_nm" class="checkbox">
					<label for="author_group_nm" class="ml_10"> 권한그룹명</label>
				</div>
...
<!-- modal : 권한그룹 검색  -->
<div id="authListPopup" class="example_content">
	<div class="popup_box">
		<!--검색박스 -->
		<div class="search_box mb_20">
			<div class="search_in">
				<div class="comm_search mr_50">
					<div class="title">권한그룹명</div>
					<input type="text" name="srchAuthNm" id="srchAuthNm" class="w_200px input_com mr_40" value="${srchAuthNm}">
					<div class="search_btn2" onclick="fnAuthPopup('B');"></div>
				</div>
			</div>
		</div>
		<div class="com_box mb_20">
			<div class="tb_outbox">
				<table class="tb_list2" id="TableID1">
					...
					<thead>
						<tr>
							<th><input type="checkbox" id="chkAuthAll" name="chkAuthAll" class="mt_5"><label for="chkAuthAll" class="ml_5">전체</label></th>
							<th>권한그룹명</th>
						</tr>
					</thead>
					<tbody id="tbAuthList">
		...
		<div class="c_btnbox">
			<div style="display: inline-block;">
				<button type="button" class="comm_btn mr_5" onclick="selAuth();">선택</button>
