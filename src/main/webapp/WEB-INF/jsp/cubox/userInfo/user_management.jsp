...
function fnUserEditPopup(uid, partcd1, cdno) {
	...
	openPopup("", "winUserEditPopup", 930, 600);
	$("#frmUserEditPop").submit();
}

function fnUserAuthorPopup(uid, partcd1, cdno) {
	$("#frmUserAuthorPop input[name='fuid']").val(uid);
	$("#frmUserAuthorPop input[name='fpartcd1']").val(partcd1);
	$("#frmUserAuthorPop input[name='sfcdno']").val(cdno);
	
	openPopup("", "winUserAuthorGroupPopup", 950, 630);
	$("#frmUserAuthorPop").submit();
}
...
<div class="totalbox">
	...
	<div class="r_btnbox  mb_10">
		<button type="button" class="btn_middle color_basic" onclick="openPopup('/userInfo/userAddPopup.do','winUserAddPopup',900,605)">신규등록</button>
		...
		<table class="tb_list">
			...
			<c:choose>
				<c:when test="${userInfoList == null || fn:length(userInfoList) == 0}">
					<tr>
						<th class="h_35px" colspan="13">조회 목록이 없습니다.</th>
					</tr>
				</c:when>
				<c:otherwise>
					...
					<tr>
						...
						<td><c:choose>
							<c:when test="${uList.fbioyn eq 'Y'}">등록</c:when>
						...
						<td><c:choose>
								<c:when test="${uList.fissyn eq 'N' && uList.fustatus eq 'Y' && (uList.cfstatus eq 'Y' || uList.cfstatus eq 'W')}"><c:set var="btnColor" value="color_basic" /></c:when>
								<c:otherwise><c:set var="btnColor" value="color_gray" /></c:otherwise>
							</c:choose>
							<button type="button" class="btn_small <c:out value="${btnColor}"/>" onclick="fnUserAuthorPopup('<c:out value="${uList.fuid}"/>', '<c:out value="${uList.fpartcd1}"/>', '<c:out value="${uList.cfcdno}"/>');">권한</button>
						</td>
	...
	<div class="popup_box">
		...
		<div class="search_box_popup mb_20">
			<div class="search_in">
				...
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="fregdts" value="fregdt" class="checkbox">
					<label for="fregdts" class="ml_10"> 등록일자</label>
				</div>
			</div>
		</div>
		<div class="search_box_popup mb_20">
