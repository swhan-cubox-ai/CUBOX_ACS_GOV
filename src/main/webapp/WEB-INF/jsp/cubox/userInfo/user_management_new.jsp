...
function userSearch() {
	...
	f.action = "/userInfo/userMngmt2.do";
	showLoading();
	f.submit();
}
...
$(function() {
	...
	$("#reset").click(function() {
		...
		$("#srchFustatus option:eq(1)").prop('selected', 'selected');
		$('.search_box input[type="checkbox"]').prop("checked",false).trigger('change');
		$("#srchFexpireyn").prop("checked",true).trigger('change');
	...
	//sort 설정
	if(!fnIsEmpty($("#hidSortName").val()) || "${userInfoVO.hidSortName}" != "") {
	...
	//modal popup set
	modalPopup ("excelAllDownloadPopup", "전체 엑셀 다운로드", 400, 700);
	modalPopup ("excelSelDownloadPopup", "선택 엑셀 다운로드", 400, 700);
	...
	$("#formPopup").on("click", function(event){
		$("#excelUploadPopup").PopupWindow("open");
	});		

	$("#excelSelDownload").on("click", function(event){
		if($("input[name=checkbx]:checked").length == 0) {
			alert("다운로드할 출입자를 체크하세요.");
			return;
		}
	...
	$("#btnDelUsers").on("click", function(event) {
		var checkId = $("input[name=checkbx]:checked");
		if(checkId.length < 1) {
			alert("삭제할 출입자를 선택하여 주십시오.");
			return;
		}
	...
	$("#btnDelUsersAll").on("click", function(event) {
		var bAct = true;
		var checkId = $("input[name=checkbx]:checked");
		if(checkId.length < 1) {
			alert("삭제할 출입자를 선택하여 주십시오.");
		...
		checkId.each(function(i) {
			var tr = checkId.parent().parent().eq(i);
			var td = tr.children();
			var status = td.eq(12).text();
			if(status.indexOf("삭제") < 0) {
				alert("출입자상태가 삭제가 아니면 영구삭제할 수 없습니다.");
				bAct = false;
				return false;				
			}
		});	
		if(bAct) {
			if(confirm("정말 영구삭제하시겠습니까?")) {
				fnDelUsers('A');
			}	
		}
	});
...
function fnDelUsers(str) {
	...
	checkId.each(function(i) {
		var tr = checkId.parent().parent().eq(i);
		var td = tr.children();
		arrId.push(td.eq(1).text());
	});

	var sUrl = "/userInfo/userListDel.do";
	if(str == "A") {
		sUrl = "/userInfo/userListDrop.do";
	}
	showLoading();
	$.ajax({
		type : "POST",
...
function fnUserAddPopup() {
<c:if test="${sessionScope.loginVO.author_id eq '00008'}">
	$("#frmUserEditPop").attr("target", "_self");
	$("#frmUserEditPop").attr("action", "/userInfo/userAddPopup2.do");
	$("#frmUserEditPop").submit();	
</c:if><c:if test="${sessionScope.loginVO.author_id ne '00008'}">
	openPopup('/userInfo/userAddPopup2.do','winUserAddPopup2',1050,720);
</c:if>
}

function fnUserEditPopup2(uid, partcd1, cdno) {
...
<c:if test="${sessionScope.loginVO.author_id eq '00008'}">
	...
	openPopup("", "winUserEditPopupNew", 1080, 720);
	$("#frmUserEditPop").attr("target", "winUserEditPopupNew");	
</c:if>
	$("#frmUserEditPop").attr("action", "/userInfo/userEditPopup2.do");	
	$("#frmUserEditPop").submit();	
}
...
function cancel(){
	...
	$("#excelUploadPopup").PopupWindow("close");
}
...
function excelUpload(){
	...
	$.ajax({
		url: "<c:url value='/userInfo/excelUserSave2.do' />",
		data: formData,
		dataType:'json',
		processData: false,
		contentType: false,
...
function fnResnChk (pchk) {
	var downresn = pchk.val().replace(/\s/gi, "");
	if(fnIsEmpty(downresn) || downresn.length < 3) {
		alert("다운로드 사유를 3자 이상 입력하여 주십시오.");
		pchk.focus();
		return false;
	}
	$("#fdownresn").val(pchk.val().trim());
	return true;
}

function excelFormDown() {
	location.href = "/userInfo/excelFormDown2.do";
}
...
function fnDownLogPop () {
	openPopup('/logInfo/downloadLogPopup.do','다운로드이력목록', 900, 575);
}
...
function selectExcelDownload(pty) {
	...
	if(pty == "A" && chkValueArray.length == 0) {
		alert("한 개 이상 체크를 하셔야 엑셀 다운로드 가능합니다.");
		return;
	} else {
...
<div class="search_box mb_20">
	<div class="search_in_bline">
		<div class="comm_search mr_20">
			<select name="srchCond" id="srchCond" size="1" class="w_100px input_com">
				<c:forEach items="${centerCombo}" var="code" varStatus="status">
					<option value='<c:out value="${code.siteId}"/>' <c:if test="${userInfoVO.srchCond eq code.siteId}">selected</c:if>><c:out value="${code.siteNm}"/></option>
				</c:forEach>
			</select>
		</div>
		<div class="ch_box  mr_10">
		...
		<div class="comm_search mr_20">
			<input type="text" class="w_142px input_com" id="srchCondWord" name="srchCondWord" placeholder="FID/이름/카드번호" value='<c:out value="${userInfoVO.srchCondWord}"/>' />
		</div>
		<div class="comm_search  mr_10">
			<input type="checkbox" id="srchFpartnm1" name="srchFpartnm1" class="checkbox" value="fpartnm1" <c:if test="${userInfoVO.srchFpartnm1 eq 'fpartnm1'}">checked</c:if>>
			<label for="srchFpartnm1"> 회사</label>
			<input type="checkbox" id="srchFpartnm2" name="srchFpartnm2" class="checkbox" value="fpartnm2" <c:if test="${userInfoVO.srchFpartnm2 eq 'fpartnm2'}">checked</c:if>>
			<label for="srchFpartnm2" class="ml_10"> 부서</label>
		</div>
		<div class="comm_search mr_20">
			<input type="text" class="w_142px input_com" id="srchPartWord" name="srchPartWord" placeholder="회사/부서" value='<c:out value="${userInfoVO.srchPartWord}"/>' />
		</div>
		<div class="ch_box  mr_20">
			<input type="checkbox" id="srchFexpireyn" name="srchFexpireyn" class="checkbox" value="N" <c:if test="${userInfoVO.srchFexpireyn eq 'N'}">checked</c:if>>
			<label for="srchFexpireyn" class="ml_10"> 유효기간 만료자 제외</label>
		</div>
		<div class="comm_search mr_10">
	...
	<div class="search_in_bline">
		...
		<div class="comm_search">
			<div class="title ">출입자상태</div>
			<select name="srchFustatus" id="srchFustatus" size="1" class="w_100px input_com">
				<option value="">전체</option>
				<c:forEach items="${userStatus}" var="code" varStatus="status">
					<option value='<c:out value="${code.fkind3}"/>' <c:if test="${code.fkind3 eq userInfoVO.srchFustatus}">selected</c:if>><c:out value="${code.fvalue}"/></option>
				</c:forEach>
			</select>
		</div>
...
<div class="totalbox">
	...
	<div class="r_btnbox  mb_10">
		<button type="button" class="btn_middle color_basic" onclick="fnUserAddPopup()">신규등록</button>
		<c:if test="${sessionScope.loginVO.author_id eq '00001'}">
			<c:choose>
				<c:when test="${userInfoVO.srchFustatus eq 'D'}"><%-- 검색조건 중 출입자상태가 '삭제'인 경우 활성화 --%>
				<button type="button" class="btn_middle color_basic" id="btnDelUsersAll">출입자 영구삭제</button>
				</c:when><c:otherwise>
				<button type="button" class="btn_middle color_basic" id="btnDelUsers">출입자삭제</button>
				</c:otherwise>
			</c:choose>
		</c:if>
		<button type="button" class="btn_middle color_basic" onclick="excelFormDown()">엑셀양식 다운로드</button>
		<button type="button" class="btn_middle color_basic" data-toggle="modal" id="formPopup">엑셀 업로드</button>
		...
		<table class="tb_list">
			...
			<thead id="thHead">
				<tr id="trUserMgt">
					...
					<th id="thfuid">
						...
						FID
						<span id="spfuid" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thfunm">
					...
					<th id="thfbioyn">
						...
						사진
						<span id="spfbioyn" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thcfcdno">
						<input type="hidden" id="sortcfcdno" name="thsortnm" />
						카드번호
						<span id="spcfcdno" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thfutypenm">
						<input type="hidden" id="sortfutypenm" name="thsortnm" />
						출입자타입
						<span id="spfutypenm" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thcfstatusnm">
						<input type="hidden" id="sortcfstatusnm" name="thsortnm" />
						카드상태
						<span id="spcfstatusnm" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thfauthtypenm">
						<input type="hidden" id="sortfauthtypenm" name="thsortnm" />
						권한타입
						<span id="spfauthtypenm" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thfpartnm1">
						<input type="hidden" id="sortfpartnm1" name="thfpartnm1" />
						회사
						<span id="spfpartnm1" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thfpartnm2">
						<input type="hidden" id="sortfpartnm2" name="thfpartnm2" />
						부서
					...
					<th id="thcfsdt">
						...
						출입시작일
						<span id="spcfsdt" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thcfedt">
						<input type="hidden" id="sortcfedt" name="thsortnm" />
						출입만료일
						<span id="spcfedt" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thfustatusnm">
						<input type="hidden" id="sortfustatusnm" name="thfustatusnm" />
						출입자상태
						<span id="spfustatusnm" style="float: right; margin-right: 5px;"></span>
					</th>
					<th id="thfregdt">
						<input type="hidden" id="sortfregdt" name="thfregdt" />
						등록일자
						<span id="spfregdt" style="float: right; margin-right: 5px;"></span>
					</th>
			...
			<c:choose>
				<c:when test="${userInfoList == null || fn:length(userInfoList) == 0}">
					<tr>
						<td class="h_35px" colspan="14">조회 목록이 없습니다.</td>
					</tr>
				</c:when>
				<c:otherwise>
					...
					<tr>
						...
						<td><a href="#none" onclick="fnUserEditPopup2('<c:out value="${uList.fuid}"/>', '<c:out value="${uList.fpartcd1}"/>', '<c:out value="${uList.cfcdno}"/>');"><c:out value="${uList.fuid}"/></a></td>
						<td><a href="#none" onclick="fnUserEditPopup2('<c:out value="${uList.fuid}"/>', '<c:out value="${uList.fpartcd1}"/>', '<c:out value="${uList.cfcdno}"/>');"><c:out value="${uList.funm}"/></a></td>
						<td><c:choose>
							<c:when test="${uList.fbioyn eq 'U'}">등록</c:when>
							<c:otherwise>-</c:otherwise>
						</c:choose><!--<c:out value="${uList.bfsstatus}"/>--></td>
						...
						<td><c:if test="${uList.fustatus eq 'Y'}"><c:out value="${uList.fustatusnm}"/></c:if>
							<c:if test="${uList.fustatus ne 'Y'}"><font class="font-color_H"><c:out value="${uList.fustatusnm}"/>(+<c:out value="${uList.fumoddays}"/>)</font></c:if>
						</td>
						<td><c:out value="${fn:substring(uList.fregdt,0,10)}"/></td>
...
<form id="frmUserEditPop" name="frmUserEditPop" method="post">
	<input type="hidden" name="fuid">
	<input type="hidden" name="fpartcd1">
	<input type="hidden" name="sfcdno">
</form>

<form id="frmUserAuthorPop" name="frmUserAuthorPop" method="post" target="winUserAuthorGroupPopup" action="/userInfo/userAuthorGroupPopup.do">
	<input type="hidden" name="fuid">
	<input type="hidden" name="fpartcd1">
	<input type="hidden" name="sfcdno">
</form>
		...
		<div class="search_box_popup mb_20">
			<div class="search_in">
				...
				<div class="mr_m3 mt_15">
					...
					<label for="cfsdt" class="ml_10"> 출입시작일</label>
				</div>
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="cfedt" value="cfedt" class="checkbox">
					<label for="cfedt" class="ml_10"> 출입만료일</label>
				</div>
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="fustatusnm" value="fustatusnm" class="checkbox">
					<label for="fustatusnm" class="ml_10"> 출입자상태</label>
				</div>
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="fregdt" value="fregdt" class="checkbox">
					<label for="fregdt" class="ml_10"> 등록일자</label>
				</div>				
			</div>
		</div>
		<div class="search_box_popup mb_20">
	...
	<div class="popup_box">
		...
		<div class="search_box_popup mb_20">
			<div class="search_in">
				...
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="fpartnm1s" value="fpartnm1" class="checkbox">
					<label for="fpartnm1" class="ml_10"> 회사</label>
				</div>
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="fpartnm2s" value="fpartnm2" class="checkbox">
					<label for="fpartnm2" class="ml_10"> 부서</label>
				</div>				
				<div class="mr_m3 mt_15">
				...
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="fustatusnms" value="fustatusnm" class="checkbox">
					<label for="fustatusnms" class="ml_10"> 출입자상태</label>
				</div>
				<div class="mr_m3 mt_15">
					<input type="checkbox" name="excelColumn" id="fregdt" value="fregdt" class="checkbox">
					<label for="fregdt" class="ml_10"> 등록일자</label>
...
<!-- 엑셀업로드 -->
<div id="excelUploadPopup" class="example_content">
<form id="fmExcel" name="fmExcel" method="post" enctype="multipart/form-data" onsubmit="return false;">
	<div class="popup_box">
		<div class="search_box_popup" style="border-bottom: 0px">
			<div class="search_in">
		...
		<div class="search_box_popup mb_20">
			<div class="search_in">
				<div class="custom-file comm_search">
					<input type="file" id="customFile" class="w_100p input_com" accept=".xls, .xlsx">
		...
		<div class="c_btnbox">
			<div style="display: inline-block;">
				<button type="button" class="comm_btn mr_5" onclick="excelUpload();">업로드</button>
				<button type="button" class="bk_color comm_btn" onclick="cancel();">취소</button>
			</div>
		</div>
	</div>
